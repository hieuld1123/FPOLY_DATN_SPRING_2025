...
public interface HoaDonRepository extends JpaRepository<HoaDon, Integer> {
    ...
    @Query("SELECT COALESCE(SUM(h.tongTien), 0) FROM HoaDon h WHERE CAST(h.ngayTao AS date) = CAST(:today AS date) AND h.trangThai = 'Thanh toán thành công'")
    Float getDoanhThuHomNay(@Param("today") Date today);

    @Query("SELECT COUNT(h) FROM HoaDon h WHERE CAST(h.ngayTao AS date) = CAST(:today AS date) AND h.trangThai = 'Thanh toán thành công'")
